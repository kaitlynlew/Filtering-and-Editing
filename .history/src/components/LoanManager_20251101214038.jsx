...
// Component for the Loan Creation Form
function LoanForm({ books, loans, onLoanBook }) {
    const [borrower, setBorrower] = useState("");
    const [selectedIsbn, setSelectedIsbn] = useState("");
    const [loanPeriod, setLoanPeriod] = useState(1); // Default 1 week

    // Books available for loan: those whose ISBN is not in the loans list
    ...
    // Set a default book selection when the component loads or when available books change
    useEffect(() => {
        if (availableBooks.length > 0 && selectedIsbn === "") {
            setSelectedIsbn(availableBooks[0].isbn13);
        }
        // If the currently selected book is now on loan, clear the selection
    ...
    const handleSubmit = (e) => {
        ...
        // Calculate due date (current date + loanPeriod weeks)
        ...
        // Reset borrower field after successful loan
        setBorrower("");
    };
...
// Component for the List of Loaned Books
function LoanList({ books, loans, onReturnBook }) {
                    ...
                    {loanedBooksDetails.map(loan => (
                        <li key={loan.isbn13} className="loan-item">
                            ...
                            <button 
                                ...
                                style={{width: '100px', height: 'auto', fontSize: '0.8rem', padding: '5px 10px', backgroundColor: '#a81c1c'}}
                                onClick={() => onReturnBook(loan.isbn13)}
                            >
                                Return
                            </button>
                        </li>
...
function LoanManager({ books, loans, onLoanBook, onReturnBook, onSwitchView }) {
    return (
        <div className="loan-manager-content">
            <div className="loan-header-controls">
                <button className="delete-btn" onClick={() => onSwitchView('catalog')}>
                    &larr; Back to Book Catalog
                </button>
            </div>
            <LoanForm books={books} loans={loans} onLoanBook={onLoanBook} />
            <LoanList books={books} loans={loans} onReturnBook={onReturnBook} />
        </div>
    );
}
